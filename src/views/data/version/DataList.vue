...
import { getDataVersionList, updateDataVersion } from '@/requests/data-version'
import { getDataSchemaList, applyDataVersion } from '@/requests/data-schema'
...
const columns = [
	{ title: '버전', key: 'data_version.version', align: 'center', width: 80 },
	{
		title: '상태',
		key: 'data_version.status',
		align: 'center',
		width: 80,
		render(row) {
			return h(
				NTag,
				{ type: dataVersionStatusObj[row.data_version.status]?.type, size: 'small' },
				{ default: () => dataVersionStatusObj[row.data_version.status]?.name }
			)
		}
	},
	{
		title: '메모',
		key: 'data_version.memo',
		render(row) {
			return h(
				NButton,
				{
					type: 'warning',
					text: true,
					onClick: () => onOpenMemoFormModal(row.data_version),
					style: row.data_version.memo_title ? '' : 'font-size: 20px'
				},
				{
					default: () =>
						row.data_version.memo_title
							? row.data_version.memo_title
	...
	},
	{
		...
		width: 200,
		render(row) {
			if (row.data_version.status === dataVersionStatusEnum.complete)
	...
	{
		title: '생성일',
		key: 'data_version.created_at',
		align: 'center',
		width: 160,
		render(row) {
			return row.data_version.created_at
				? dayjs(row.data_version.created_at).tz().format('YYYY-MM-DD HH:mm:ss')
				: ''
		}
	},
	{
		...
		render(row) {
			return [
				row.data_version.status === dataVersionStatusEnum.error
					? errorButton(row)
					: dataTable.value.type === dataTableTypeEnum.data
					? applyButton(row)
					: null,
...
const errorModalData = ref({ visible: false, title: '', message: '' })
const memoFormModal = ref({ visible: false, form: { id: 0, memo_title: '', memo: '' } })

const applyServersTag = (row) => {
	return row.data_version.servers?.map((server) => {
		...
		NDropdown,
		{
			...
			onSelect: (serverId) =>
				serverId === -1
					...
					: onApplyVersion(serverId, row.data_version.id)
		},
		{
			default: () =>
				h(
...
const errorButton = (row) => {
	return h(
		NButton,
		{
			...
			onClick: () =>
				onOpenErrorModal(
					`${dataTable.value.sheet_name} v.${row.data_version.version} by ${row.user.name}`,
					row.data_version.error
				)
		},
		{
			default: () => '에러'
...
const onOpenErrorModal = (title, message) => {
...
const onCloseErrorModal = () => {
	errorModalData.value = { visible: false, title: '', message: '' }
}
...
const onSubmitMemoForm = async (e) => {
	e.preventDefault()
	loading.value = true
	const res = await updateDataVersion(memoFormModal.value.form)
		.then((r) => r.data)
		.catch(() => {
			loading.value = false
		})
...
const onApplyVersion = async (serverId, versionId) => {
	loading.value = true
	const res = await applyDataVersion(serverId, versionId)
		.then((r) => r.data)
		.catch(() => {
			loading.value = false
		})
...
const setAppliedServer = () => {
	data.value.forEach(({ data_version }) => {
		data_version.servers = []
		dataSchemaList.value.forEach((dataSchema) => {
			if (dataSchema.version) {
				const versionObj = JSON.parse(dataSchema.version)
				if (versionObj[dataTable.value.name] == data_version.version) {
					data_version.servers.push(dataSchema.server)
				}
			}
		})
	})
...
<template>
	...
	<n-modal
		v-model:show="errorModalData.visible"
		:title="errorModalData.title"
		:mask-closable="false"
		:show-icon="false"
		preset="card"
		@close="onCloseErrorModal"
		@esc="onCloseErrorModal"
		style="width: 100%; height: 100vh"
	>
		<JsonViewer
			:value="errorModalData.message"
			:expanded="true"
			:expand-depth="3"
			copyable
			boxed
